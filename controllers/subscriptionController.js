...
const paymentService = require('../services/paymentService');
const PLANS = require('../config/plans');
const InvestorProfile = require('../models/InvestorProfile'); // 👈 Using InvestorProfile
const { sendEmail } = require('../services/emailService');
...
const subscribeToPlanCrypto = asyncHandler(async (req, res) => {
    ...
    if (slug) {
        // Find plan by matching slug
        const plan = Object.values(PLANS).find(p => p.slug === slug);
        if (!plan) { res.status(400); throw new Error(`Invalid plan selected: ${slug}`); }
        
        // Use direct plan.price from your updated config structure
